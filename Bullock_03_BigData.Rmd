...
```{r eval=FALSE}
gflu = read.csv("http://apps.fs.fed.us/fiadb-downloads/RI_SEEDLING.CSV")
abundance=log(sort(tapply(gflu$SPCD,gflu$TREECOUNT,sum,nr.rm=TRUE), decreasing=TRUE))
x <- seq(1,length(abundance))
plot(x,abundance,log="y",xlab="Ranking",ylab="Abundance",main="Rhode Island Seedling Abundance")
```
...
```{r}
fluxnets <- getURL("http://fluxnet.ornl.gov/site_status")
flux_table <- readHTMLTable(fluxnets)[[1]]
ID <- flux_table[,"FLUXNET ID"]
Countries <- substring(ID,1,2)
Table <- sort(table(Countries),decreasing=TRUE)
print(Table)
```
...
```{r}
numb=grep("^#",myCode) #The indices for the matching lines
myCode[numb] #The matching lines
```
...
```{r eval=FALSE}
file=nc_open("http://thredds.daac.ornl.gov/thredds/dodsC/ornldaac/1220/mstmip_driver_global_hd_climate_tair_2004_v1.nc4")
print(file) # Get variable information
lat=ncvar_get(file,"lat")
lon=ncvar_get(file,"lon")
time=ncvar_get(file,"time")
time_length=length(time)
boston_lat=which(lat>42 & lat < 42.5) #Roughly Boston's Latitude
boston_long=which(lon>71 & lon<71.5) #Roughly Boston's Longitude
boston_at=ncvar_get(file,"tair",c(boston_long, boston_lat,1),c(1, 1,time_length))
plot(time,boston_at,xlab="Days Since 1700-01-01",ylab="Mean Temperature (K)", main="Boston Mean Temperature")
nc_close(file)
```
----
...
```{r}
par(mar = c(5,5,2,5))
plot(time,EVI,col='red',type='p',lwd=5,xlab='Date')
par(new = T)
plot(doy,filter(NEE[3,],rep(1/24,24)),type='l',main=paste("Height =",height[3],"m"),axes=F,xlab=NA,ylab=NA) #Plotting just one height
axis(side=4)
mtext(side = 4, line = 3, 'Net Ecosystem Exchange CO2')
```
...
Note that this is how it would be done it R. It does not need to be done in R, but this is one way to do it. 

```{r eval=FALSE}


check_date <- function(curdate, database){
   fia_html <- getURL(database)  
   fia_table <- readHTMLTable(fia_html)[[6]]   
   datasets <- length(fia_table[,"CSV Files"])
   for (i in seq(1,datasets)) {
     update = julian(as.Date(fia_table[,"Last Modified Date"][i]))
     if (curdate > update) { 
        csv_name <- fia_table[,"CSV Files"][i] ##The CSV that is out of date
        url_name<-paste('http://apps.fs.fed.us/fiadb-downloads',csv_name,sep="") ##The URL of the out of date CSV
        getURL(url_name,csv_name,mode="w") ##Download the csv. 
      }
   }
}

curdate=as.Date(Sys.Date()) ##Get the current date
database<-'http://apps.fs.fed.us/fiadb-downloads/datamart.html' ##The database we are using
check_date(curdate,database)  ##Run the function
...
```{r eval=FALSE}
MAILTO=bullocke@bu.edu
01 01 * * 1 /Users/ericbullock/Google Drive/Class/Ecological_Forecasting/Update_Database.R
```
