...
```{r}
iter=list(1.95, 2.05, 2.5, 2.8)
for (it in (iter)) {

#Iterate simulation
  n = rep(n0,NT)    ## vector to store results
  for(t in 2:NT){
  n[t] = n[t-1] + it*n[t-1]*(1-n[t-1]/K)
  }

#Plot the results
  plot(time,n,ylim=c(0,12),lwd=3,type='l',
     ,bty='l',cex.lab=1.5,
      xlab="Time",ylab=paste("Population Size at r=", it, 1, sep = ""))
}
```

#Explanation
1.95: Lowest growth rate of the plots. This causes a slight variance in the projection once it hits carrying capacity, but it looks to be evening out.

2.05: There is a more uniform variance after the population hits carrying capacity and it does not appear to be evening out. 

2.5: There is a large variance after carying capacity. The variance starts off small then drastically increases and becomes steady. 

2.8: The variance after carrying capacity is around the same as with 2.5, but it immedietely becomes that large. 
...
I am choosing the Weibull distribution

```{r}
x = seq(0,100,by=1)
plot(x,dweibull(x, 5,50),type='l')    	## that’s a lowercase “L” for “line”

```

```{r}
#Cumulative distribution function
plot(x,pweibull(x, 5, 50), type='l')
```

```{r}
#Quantile function
plot(x,qweibull(seq(0,1,by=.01), 5, 50), type='l')
```

```{r}
#Histogram
x = seq(0,1000,by=1)
hist(rweibull(x,5,10),probability=TRUE,breaks=100) 
lines(x,dweibull(x, 5,10),type='l',col='red') 
```
...
x = rlnorm(10000,0,.5)
y = sin(x)
x.stats = quantile(x,c(0.025,0.5,0.975))
y.stats = quantile(y,c(0.025,0.5,0.975))
hist(x,main="Original distribution",breaks=40)

abline(v=quantile(x,c(0.025,0.5,0.975)),lty=c(2,1,2),lwd=3,col="orange")
abline(v=mean(x),col="red",lwd=3,lty=3)
text(5,700,sprintf("Mean = %.3f", mean(x)))
text(5,550,sprintf("Median = %.3f", x.stats[2]))
text(5,400,sprintf("95%% CI = %f, %f", x.stats[1],x.stats[3]))

h2=hist(y,main="Transformed distribution",breaks=40)
abline(v=quantile(y,c(0.025,0.5,0.975)),lty=c(2,1,2),lwd=3,col="orange")
abline(v=mean(y),col="red",lwd=3,lty=3)
text(-.5,1500,sprintf("Mean = %.3f", mean(y)))
text(-.5,1350,sprintf("Median = %.3f", y.stats[2]))
text(-.5,1200,sprintf("95%% CI = %f, %f", y.stats[1],y.stats[3]))
```
...
```{r}
hist(rE,main="Sample r", breaks=50)
hist(KE,main="Sample K", breaks=50)
```
...
subset<-sample(1:NE,10,replace=TRUE) #get random subset
colors <- rainbow(10) #Get 10 random colors
plot(time,n[subset[1],],lwd=3,type='l',xlab="Time",main="Logistic Growth",ylab="Population Size",col=colors[1],ylim=c(0,15))
for (i in 2:10){
  lines(time,n[subset[i],],lwd=3,type='l',col=colors[i])
}
```
...
```{r}
hist(n[,15],xlab="Population Size",main="Population Size at Time = 15")
```

7. Plot the median trajectory and 95% CI.

```{r eval=FALSE}
plot(time,n.stats[2,],ylim=c(0,max(n.stats[3,])),type='l',lwd=3,xlab="Time",ylab="Population Size")
ciEnvelope(time,n.stats[1,],n.stats[3,])
lines(time,n.stats[2,],type='l',lwd=3)
```
...
```{r eval=FALSE}
n.stats2 = apply(n,2,quantile,c(0.25,0.5,0.75))
ciEnvelope(time,n.stats2[1,],n.stats2[3,],col='light blue')
lines(time,n.stats[2,],type='l',lwd=3)
legend("bottomright",lwd=5,lty=1,c('Median','95% CI','50% CI'),col=c('black','grey','light blue'))
```
...
n.stats = apply(n,2,quantile,c(0.025,0.5,0.975))
plot(time,n.stats[2,],ylim=c(0,max(n.stats[3,])),type='l',lwd=3,xlab="Time",ylab="Population Size",main="r=.3")
ciEnvelope(time,n.stats[1,],n.stats[3,])
lines(time,n.stats[2,],type='l',lwd=3)
...
for (it in (iter)) {
  ...
  n.stats = apply(n,2,quantile,c(0.025,0.5,0.975))
  plot(time,n.stats[2,],ylim=c(0,max(n.stats[3,])),type='l',lwd=3,xlab="Time",ylab="Population Size",main=sprintf("r=%.3f",it))
  ciEnvelope(time,n.stats[1,],n.stats[3,])
  lines(time,n.stats[2,],type='l',lwd=3)
}
