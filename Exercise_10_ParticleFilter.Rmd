...
#                log(params$alpha*(1-exp(-0.5*LAI))*inputs$PAR),
#                0)
  
  if(inputs$PAR>1e-20){
      GPP = log(params$alpha*(1-exp(-0.5*LAI))*inputs$PAR)
  } else {
      GPP = rep(0,ne)
  }
  ...
  Rh = pmin(log(params$Rbasal*X[,3]*params$Q10^(inputs$temp/10)),X[,3]/k) ## pmin ensures SOM never goes negative
...
#  return(cbind(X1=X1,X2=X2,X3=X3,LAI,NEP=GPP-alloc[,1]-Rh,GPP=GPP,
...
for(t in 1:nt){
  ...
  ## forward step
  output[t,,]=as.matrix(SSEM(X,params,inputs[t,]))
  X=output[t,,1:3]
