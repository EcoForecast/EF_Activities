...
Finally, since growth is indexed by both tree and year, lets add random effect for both individuals and years. In this case our process model now becomes   Dnew[i,t] <- x[i,t-1] + mu + ind[i] + year[t], where ind and year are the random effects for individual and year respectively. Next, we'll need to specify the distributions that these random effects are drawn from, as well as the priors on the random effect variances
