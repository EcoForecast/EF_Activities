...
```{r}
## split data
dfa <- df %>% filter(start_time < lubridate::as_date("2022-01-01"))
dfb <- df %>% filter(start_time >= lubridate::as_date("2022-01-01"))
## write out to the folder "parquetTest", tiling by start_time
dir.create("parquetTest")
dfa %>% group_by(start_time) %>% arrow::write_dataset("parquetTest",format="parquet")
dfb %>% group_by(start_time) %>% arrow::write_dataset("parquetTest",format="parquet")
```
