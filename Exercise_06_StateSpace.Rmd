...
where $\beta_0$ is the intercept, $\beta_1$ is the slope of the covariate effect, and $\beta_{IC}$ is the slope of the initial condition effect. Rather than implement this model in JAGS directly, we're going to rely on the ecoforecastR package, which accepts a `lm` like syntax for specifying covariates.

```{r}
if(file.exists("data/bostonMet.RData")){
  load("data/bostonMet.RData")
} else {
## get weather data as covariate
station_data <- rnoaa::ghcnd_stations()
boston <- rnoaa::meteo_nearby_stations(data.frame(id="boston",latitude=42.36,longitude=-71.06),station_data = station_data,radius=20)
Tair = rnoaa::meteo_pull_monitors(boston$boston$id,date_min = min(time),date_max = max(time),var = "TAVG")

save(Tair,"data/bostonMet.RData")
}

#Tair = rnorm(length(data$y),data$y,0.5) ## pseudodata, replace with real
data$Tair = Tair

## fit the model
ef.out <- ecoforecastR::fit_dlm(model=list(obs="y",fixed="~ Tair"),data)
