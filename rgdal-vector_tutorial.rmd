...
The objective of this tutorial is to show some basic functionality to the rgdal package to deal with vector data in R. This tutorial assumes that GDAL is properly installed in the system. For options depending on which OS you are using, you can check [here](https://trac.osgeo.org/gdal/wiki/DownloadingGdalBinaries) 
...
if (file.exists('training_data.shp') == F) {
    download.file(url = 'https://raw.githubusercontent.com/parevalo/EF_Activities/master/vector/training_data.zip', 
                  destfile = 'training_data.zip', method = 'auto')
...
The function `summary` shows detailed information about the file:
```{r}
summary(training)
```

The file has three fields, corresponding to "id", "class" (which is numerical, for now) and "Comment", which is a string field. You can also see the extent coordinates and the spatial projection. If we only want to see information contained in the attribute table we could use:

```{r}
head(training@data)
```

We can also see the overall structure of the file:
```{r}
...
We can also plot along with other files, such as a raster stack. First we will download an example raster and then we will do the plotting on top of it. These two files are in the same projection, but reprojection would be needed otherwise.

```{r}
library(raster)
if (file.exists('LE70070592003004_clip.tif') == F) {
    download.file(url='https://raw.githubusercontent.com/parevalo/EF_Activities/master/LE70070592003004_clip.tif',
                  destfile='LE70070592003004_clip.tif', method='auto')
}
...
```{r}
writeOGR(training_wgs84, dsn = "training_wgs84.kml", layer = training, driver = "KML")
```